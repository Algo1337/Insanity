...
__SKID_INVALID_ARG_ERR__ = discord.Embed(title = "Force List Of Skids Lockup", description = "Strip roles and force skid jail including upon re-joining", color = discord.Colour.red())
__SKID_INVALID_ARG_ERR__.add_field(name = "**Add Skid to Jail**", value = "```>nuke --add <@tag/userid> <?--strip>```", inline = False)
__SKID_INVALID_ARG_ERR__.add_field(name = "**Remove Skid from Jail**", value = "```>nuke --rm <@tag/userid> <?--reset>```", inline = False)
...
async def skid(base, msg: DiscordUtils) -> True:
    ...
    if opt == "--add":
        ...
        if "--strip" in msg.Args:
            ...
            await msg.send_embed("Blacklisted Skid", f"Deleting {len(all_roles)} roles from <@{userid}> and sent to jail. This will happen on re-join also!")
