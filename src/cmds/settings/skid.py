...
async def skid(base, msg: DiscordUtils) -> True:
    ...
    if opt == "--add":
        ...
        if "--l" not in msg.Args:
            ...
            await msg.send_embed("Blacklisted Skid", f"Deleting all <@{msg.get_arg(1)}> {len(all_roles)} roles and sent to jail")
